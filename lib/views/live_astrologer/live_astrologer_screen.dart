...
class _LiveAstrologerScreenState extends State<LiveAstrologerScreen> {
                                        ...
                                        Container(
                                          ...
                                          child: Padding(
                                            padding: const EdgeInsets.only(
                                                top: 8.0, bottom: 5),
                                            child: Text(
                                              '${tr("Minimum balance of 5 minutes")} (${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} $totalCharge) ${tr("is required to start call")}',
                                              style: TextStyle(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () async {
                              Get.back();
                              double totalCharge =
                                  videoCallCharge2! * time[selectTime];
      ...
      RtcEngineEventHandler(
          ...
          onUserOffline: (RtcConnection connection, int remoteUId,
            ...
            if (remoteUid == remoteUId) {
              ...
              FutureBuilder(
                future: liveController.deleteLiveAstrologer(astrologerId2!),
  ...
  Future<AgoraRtmChannel> createChannel(String name) async {
      ...
      if (liveController.isImSplitted == true) {
        liveController.getLiveuserData(channel2!).then((value) async {
          ...
          await global.callOnFcmApiSendPushNotifications(
              ...
              timeInInt: timeIn.toString(),
              joinUserName: global.user.name,
              joinUserProfile: global.user.profile.toString());
        });
      }
    };
    ...
    channel.onMessageReceived = (message, fromMember) {
      ...
      setState(() {
        ...
        debugPrint('guru pickture is ${message.text.split("&&")[2]}');
        messageList.add(MessageModel(
  ...
  firebaseChatinit() async {
    log('insde firebase chat');

    chatController
        .getMessageRTM('liveAstrologer_${widget.astrologerId}')!
        .listen((snapshot) {
      if (snapshot.docs.isNotEmpty) {
        ...
        snapshot.docs
            ...
            .forEach((message) {
          setState(() {
            debugPrint('before msg leng ${messageList.length}');
            // messageList.clear();
            messageList.add(message);
            debugPrint('after msg leng ${messageList.length}');

            reverseList = messageList.reversed.toList();
          });
          reverseList.forEach((message) {
...
//WEB SUPPORT
  sendFirebaseRTMMessage(String channelMessage, String? gift) async {
