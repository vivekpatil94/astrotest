...
class TabViewAstrologer extends StatelessWidget {
                                    ...
                                    child: Container(
                                      child: Center(
                                        child: GetBuilder<CallController>(
                                          builder: (CallController controller) =>
                                              InkWell(
                                            onTap: () async {
                                              bool isLogin = await global.isLogin();

                                              _logedIn(
                                                  context, isLogin, index, true);
                                            },
                                            child: CircleAvatar(
                                              radius: 18,
  ...
  void _checkAstrologerAvailability(index) {
    if (astrologerList[index].callStatus == "Offline") {
      global.showToast(
        message: 'Currently Offline',
        textColor: Colors.red,
        bgColor: Colors.black,
      );
    } else if (astrologerList[index].callStatus == "Busy") {
      global.showToast(
        message: 'Currently Busy',
        textColor: Colors.red,
        bgColor: Colors.black,
      );
    } else if (astrologerList[index].callStatus == "Wait Time") {
      if (astrologerList[index].callWaitTime != null) {
        int remainingMinutes = astrologerList[index]
            .callWaitTime!
            .difference(DateTime.now())
            .inMinutes;

        String message = remainingMinutes > 0
            ? "Wait till - $remainingMinutes min"
            : "Wait till";
